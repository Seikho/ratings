 ...
 */
export function chance(homeRating: number, awayRating: number): number {
...
export function expected(homeRating: number, awayRating: number, options?: Options): number {
    options = options || {};
    let scoreUpperLimit = options.scoreUpperLimit || DEFAULT_SCORE_LIMIT;
    
    let winChance = chance(homeRating, awayRating);
    
    return ((scoreUpperLimit * 2) * winChance - scoreUpperLimit) * (homeRating > awayRating ? 1 : -1) 
}
